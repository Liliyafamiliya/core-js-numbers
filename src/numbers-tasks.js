...
function roundToPowerOfTen(num, pow) {
  if (pow === 0) return num;
  let start = String(num).slice(0, -pow);
  const end = String(num).slice(-pow);
  if (end[0] > 4) {
    start = +start + 1;
  }
  const newEnd = '0'.repeat(pow);
  return +`${start}${newEnd}`;
}
 ...
 */
function isPrime(n) {
  const arrNumbers = new Array(n).fill(1).map((num, index) => num + index);
  const arrFilter = arrNumbers.filter((num) => n % num === 0);
  if (arrFilter.length === 2 && n !== 1) {
    return true;
  }
  return false;
}
...
function getRandomInteger(min, max) {
  ...
  const scaledRandom = randomFraction * range;
...
function getCountOfOddNumbers(number) {
  const arrNumbers = new Array(Math.abs(number))
    .fill(1)
    .map((num, index) => num + index);
  const arrFilter = arrNumbers.filter((num) => num % 2 > 0);
